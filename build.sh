...
build_profiler-test()
{
    ...
    "$__ProjectRoot/src/tools/gen-buildsys-clang.sh" "$__ProjectRoot" $__ClangMajorVersion $__ClangMinorVersion $__CMakeArgs
    ...
    if [ $? != 0 ]; then
        echo "Failed to build profiler-test components."
        exit 1
    fi
}
...
# This is where all built profiler-test libraries will copied to.
...
# Build the profiler-test (native) components.
